...
export default function OpenTicket() {
    ...
    async function handleSearchCustomer(data: FormData) {
        ...
        if (response.data === null) {
            setError('email', {type: 'custom', message: 'Ops, cliente não foi localizado!'})
            return
        }
            ...
            <main className="flex flex-col mt-4 mb-2">
                {customer ? (
                    <div className='bg-slate-200 py-6 px-4 rounded border-2 flex items-center justify-between'>
                        ...
                        <button
                            className='bg-red-600 h-11 px-2 rounded flex items-center justify-center'
                    ...
                    >
                        <div className='flex flex-col gap-3'>
                            <Input
                                ...
                                register={register}
                            />
                            <button type='submit' className='bg-blue-500 flex flex-row gap-4 px-2 h-11 rounded items-center justify-center text-white font-bold'>
                                Procurar cliente
